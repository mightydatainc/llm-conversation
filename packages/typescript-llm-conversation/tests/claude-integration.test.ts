...
import { CLAUDE_MODEL_VISION } from '../src/llmProviders.js';
...
describe('Claude integration (live API)', () => {
...
`);
  ...
  it('should perform image recognition with manual content message', async () => {
    ...
    const imgDataBase64 = pngBuffer.toString('base64');

    const ClaudeMsgWithImage = {
      role: 'user',
      content: [
        {
          type: 'image',
  ...
  it('should track token usage', async () => {
    ...
    const stage1 = async () => {
      ...
      expect(tokenUsage.allModels.output).toBeLessThan(800);
